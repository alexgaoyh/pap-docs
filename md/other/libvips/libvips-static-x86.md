...
sudo rm -rf /usr/local/x86_64-linux-musl
```
...
# 5、配置 PATH
```shell
export PATH=/usr/local/x86_64-linux-musl/bin:$PATH
```
...
BUILD_ROOT=/tmp/build-vips-static-x86
MAKE_JOBS=$(nproc)
PKG_DIR=/home/ubuntu/packages   # 请确保此目录下有源码包
...
# 编译参数 (针对 x86_64 优化)
export CFLAGS="-O2 -static -fno-pie -no-pie -march=x86-64"
export CXXFLAGS="-O2 -static -fno-pie -no-pie -march=x86-64 -static-libgcc -static-libstdc++"
...
echo "--- Building Image Dependencies (x86_64) ---"
# zlib
...
./configure --host=x86_64-linux-musl --enable-static --disable-shared --prefix=${PREFIX_BASE}/deps-static
make -j$MAKE_JOBS && sudo make install
cd ..
...
echo "--- Building GLib Stack (x86_64) ---"
# PCRE2
...
# GLib Meson Cross File (x86_64 version)
tar -xf ${PKG_DIR}/glib-${GLIB_VER}.tar.xz && cd glib-${GLIB_VER}
cat > x86_64_musl_cross.txt <<EOF
[host_machine]
system = 'linux'
cpu_family = 'x86_64'
cpu = 'x86_64'
...
meson setup build --prefix=${PREFIX_BASE}/deps-static --cross-file x86_64_musl_cross.txt \
...
# 2. 重新配置并编译
./configure --host=x86_64-linux-musl --enable-static --disable-shared --prefix=${DEPS_PREFIX}
make -j$MAKE_JOBS
...
# 沿用 GLib 生成的 cross file
meson setup build \
  --cross-file=../glib-2.78.0/x86_64_musl_cross.txt \
